...
| 가격 JSON 도메인(apex) | `blusedeal.co.kr` |  | 카페24 VPS로 연결 (정적 JSON: `/meta.json`, `/prices/*.json`) |
| 가격 JSON 도메인(www) | `www.blusedeal.co.kr` |  | (선택) 동일(가격 JSON) |
| (선택) 가격 API 서브도메인 | `api.blusedeal.co.kr` |  | 가능하면 분리(전용). 관리형 VPS에선 vhost 제약으로 403이 날 수 있어 생략 가능 |
...
### B) VPS(가격 JSON)
- [ ] `blusedeal.co.kr` → A 레코드: `<VPS 공인 IP>`
- [ ] `www.blusedeal.co.kr` → A 레코드: `<VPS 공인 IP>` (선택)

(선택) `api.blusedeal.co.kr`을 분리 운영한다면:
...
### nginx 설정
- server_name(가격 JSON): `blusedeal.co.kr` (및 선택 `www.blusedeal.co.kr`)
- (선택) server_name(가격 API 전용): `api.blusedeal.co.kr`
- root(예시): `/srv/bluedeal-api/public` 또는 카페24 관리형 WP라면 `/crokerda1/www`
...
### 파일 경로(권장)
- JSON 루트(예시): `/srv/bluedeal-api/public/` 또는 `/crokerda1/www/`
- 카테고리 폴더: `<JSON 루트>/prices/`
...
2. 브라우저에서 `https://blusedeal.co.kr/meta.json` 직접 열어 200인지 확인
