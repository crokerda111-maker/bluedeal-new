    ...
    <div className="space-y-6">
      <section className="bd-surface-md p-6">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          <Link
            href="/community/hotdeal/write"
            className="rounded-xl bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950"
          >
            핫딜 제보하기
          </Link>
          <div className="text-[12px] text-white/50">유저 제보는 게시판에 등록됩니다.</div>
        </div>
        </div>
      </section>
      ...
      <section className="space-y-3">
        {filtered.length === 0 ? (
          <div className="bd-surface-md p-10 text-center text-sm text-white/60">
            조건에 맞는 핫딜이 없습니다.
          </div>
        ) : (
          filtered.map((d) => (
            <div key={d.id} className="bd-surface-md p-5 hover:bg-white/10">
