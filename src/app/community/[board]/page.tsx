...
export default function CommunityBoardPage({ params }: { params: { board: string } }) {
  ...
  // If this board only allows one type, force filter to "all".
  useEffect(() => {
    if (typeOptions.length <= 1) setTypeFilter("all");
  }, [typeOptions.length]);
