...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Realtime posts (3 rows) */}
      <section>
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-sm font-semibold">실시간 게시글</div>
            <div className="mt-1 text-xs text-white/55">IT + 커뮤니티 최신 글 (상위 3개)</div>
          </div>

          <div className="flex flex-wrap gap-2">
            <Link href="/it" className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-white/80 hover:bg-white/10">
              IT 소식
            </Link>
        ...
        <div className="mt-4">
          <RealtimePosts limit={3} showHeader={false} />
        </div>
      </section>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-4">
          {QUICK_LINKS.map((c) => (
            <Link
              key={c.href}
              href={c.href}
              className="group bd-surface-md p-4 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/60"
            >
      ...
      <section>
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <div className="mt-1 text-xs text-white/55">카테고리별 핫딜을 모아서 확인할 수 있습니다.</div>
          </div>
        ...
        <div className="mt-4 grid gap-3">
          {deals.map((d) => (
            <Link
              key={d.id}
              href={`/deals/${d.id}`}
              className="group bd-surface-md p-4 hover:bg-white/10"
            >
